...
export function FileCard({ item }: { item: QueueItem }) {
  ...
  const showCheckbox = queueLength > 1;
  ...
  return (
    <div className={cn(
      "group rounded-xl border bg-card shadow-sm transition-all",
      isSelected ? "border-primary ring-2 ring-primary/20" : "border-border",
      isDone && !isSelected && "border-green-200 dark:border-green-900/50",
      isError && !isSelected && "border-destructive/30"
    )}>
      <div className="flex items-start gap-3 p-4">
        {/* Checkbox — only shown when there are multiple files */}
        {showCheckbox && (
          <button
            onClick={() => toggleSelect(item.id)}
            className={cn(
              "mt-0.5 h-4 w-4 shrink-0 rounded border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
              isSelected
          ...
          >
            {isSelected && (
              <svg viewBox="0 0 12 12" fill="none" className="h-4 w-4 -mt-0.5 -ml-0" stroke="currentColor" strokeWidth="2">
